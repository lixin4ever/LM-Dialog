...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, kv=None,
        ...
        residual = x
        # x after self_attn is actually the context vector
        if kv is None:
            # for self attention
            ...
            # for encoder-decoder attention, specifically, source attention
        ...
        # perform residual connection as well as the layer normalization
...
class AttLSTMLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, weights_dropout=True):
        """
        LSTM language model with self-attention
        :param embed_dim:
        :param num_heads:
        :param dropout:
        :param weights_dropout:
        """
        super(AttLSTMLayer, self).__init__()
        self.self_attn = MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout)
        self.attn_layer_norm = nn.LayerNorm(embed_dim)
        # bidirectional should be False for the language modeling
    ...
    def forward(self, x, kv=None, self_padding_mask=None, self_attn_mask=None,
                external_memories=None, external_padding_mask=None,
                need_weights=False):
        # apply rnn, (seq_len, bsz, hsz)-->(seq_len, bsz, hsz)
        x, _ = self.lstm(x)
        residual = x
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False):
        """ Input shape: Time x Batch x Channel
            ...
            need_weights: defaults to False
        """
        ...
        # perform weighted combination according to the weight
        # in encoder-decoder attention, attn is actually the context vector
        ...
        if need_weights:
            ...
            # attn_weights shape: (bsz, num_heads, target_len, source_len)
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            
            # attn_weights, _ = attn_weights.max(dim=1)
            # shape: (num_heads, bsz, target_len, source_len)
            attn_weights = attn_weights.transpose(0, 1)
            # keep the attention from the *FIRST* head
            # current attn_weights shape: (target_len, bsz, source_len)
            attn_weights = attn_weights[0].transpose(0, 1)
        else:
            # return None as attention weights
            attn_weights = None
